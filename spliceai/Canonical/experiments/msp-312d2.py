...
msp.architecture["influence_calculator_spec"]["long_range_reprocessor_spec"] = dict(
    ...
    tanh_attn_layer_spec=dict(
        type="OperationOnAttentionLayers",
        first_spec=dict(type="SigmoidConvolvedAttnLayer"),
        second_spec=dict(type="SigmoidConvolvedAttnLayer"),
        operation_spec=dict(type="sub"),
    ),
)
