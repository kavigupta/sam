    ...
    with open(rbns_data, 'r') as f:
        ...
        for line in f:
            ...
            if expr_target == '':
                ...
                expr_target = expr_target.split("-")[0]
            ...
            if expr_target == '':
                pass
            elif target_protein == expr_target:
                pass
            else:
                continue
...
if  __name__ == "__main__":
    ...
    with open('intermediate_data/enrichment_pentamer.txt', 'r') as f:
        for line in f:
            content = line.split(', ')
            protein_name = content[0].split('-')[0]
            enrichment_dict[protein_name] = float(content[1])
        f.close()
    print(enrichment_dict)

    # print(map_index)

    for rbns_name in rbns_name_psam_idx_dict:
        if rbns_name_psam_idx_dict[rbns_name] < 0:
            generate_dataset(target_protein=rbns_name, each_class_train_size=each_class_train_size, data_dir=data_dir, map_index=map_index, enrichment_dict=enrichment_dict)
            print(f"Generatio of {rbns_name} protein Done!")
