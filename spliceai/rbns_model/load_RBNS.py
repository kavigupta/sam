    ...
    for line in f:
        ...
        if output_type == 'enrichment':
            ...
            for data_line in enrichment_data:
                ...
                if n == 0:
                    if 'nM' not in decode_line:
                        break
                    # print(f"decode_line: {decode_line}")
                    concentration_list = re.split('\t|\n', decode_line)[:-1]
                ...
                if n == 1:
                    ...
                    best_concentration = concentration_list[1 + arg_max]
                    enrichment_dict[expr_target] = best_concentration
                    # print(enrichment_list)
                n += 1
    
print(f"enrichment_dict: \n {enrichment_dict}")
# exit(0)
    ...
    for line in f:
        ...
        if expr_target == '':
            ...
            if expr_target in enrichment_dict:
                if enrichment_dict[expr_target].split(' ')[0] != protein_concentration:
                    continue
            else:
                continue
        
        # continue
        # 186: https://www.encodeproject.org/files/ENCFF260OIA/@@download/ENCFF260OIA.fastq.gz
        # download file
        if url == 'https://www.encodeproject.org/files/ENCFF002DHC/@@download/ENCFF002DHC.fastq.gz':
            print('check and jump')
            continue
        print('start getting', url)
        ...
        if i == 0:
            ...
            h5f = h5py.File('data_20/' + 'rbns' + '_' + 'train' + '.h5', 'w')
            ...
            h5f = h5py.File('data_20/' + 'rbns' + '_' + 'test' + '.h5', 'w')
            ...
            h5f = h5py.File('data_20/' + 'rbns' + '_' + 'train' + '.h5', 'r')
            L = h5f['L'][:]
            h5f.close()
            l_set = set()
            ...
            h5f = h5py.File('data_20/' + 'rbns' + '_' + 'train' + '.h5', 'a')
            ...
            h5f = h5py.File('data_20/' + 'rbns' + '_' + 'test' + '.h5', 'a')
